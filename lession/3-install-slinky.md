Slurm Slinky는 초거대 규모의 HPC(고성능 컴퓨팅) 클러스터에서 발생하는 통신 병목 현상을 해결하기 위해 SchedMD가 설계한 차세대 마이크로서비스 기반 제어 계층 아키텍처이다. 기존의 단일 slurmctld 구조에서 벗어나 상태 관리와 메시지 전달 기능을 분리하며, 특히 쿠버네티스(Kubernetes) API를 활용해 가상화된 환경에서도 동적으로 자원을 오케스트레이션할 수 있는 확장성을 제공한다. 또한, 대규모 작업 제출 시 발생하는 부하를 분산 처리하는 고성능 메시지 버스를 도입하여 시스템의 응답성을 최적화하고, 온프레미스와 퍼블릭 클라우드 간의 경계 없는 자원 활용을 가능케 하는 하이브리드 클라우드 최적화 기술을 핵심으로 한다.
Slurm Slinky는 기존의 slurmctld에 집중되던 부하를 분산하기 위해 상태 관리 기능을 마이크로서비스 구조로 재설계한 차세대 제어 계층 아키텍처이다. 이 시스템은 기존 Slurm 바이너리 및 설정과 완벽한 하위 호환성을 유지하도록 설계되어, 사용자는 기존에 사용하던 sbatch나 srun 등의 명령어를 수정 없이 그대로 사용할 수 있다. 기술적으로는 고성능 키-값 저장소인 etcd를 활용해 클러스터의 상태 데이터를 실시간으로 동기화하며, 쿠버네티스 커스텀 리소스(CRD)를 통해 컴퓨팅 노드를 관리함으로써 자동 복구(Self-healing) 및 가상화된 자원의 유연한 할당을 지원한다. 또한, 기존의 slurmd 에이전트와 통신하는 인터페이스를 표준화하여 하이브리드 클라우드 환경에서도 온프레미스 노드와 클라우드 노드를 단일 파티션 내에서 이질감 없이 제어할 수 있는 운영 연속성을 보장한다.
