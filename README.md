...
본 워크숍은 Amazon EKS 환경에서 Slurm(Simple Linux Utility for Resource Management)을 활용하여, Llama 3와 같은 대규모 모델을 가장 비용 효율적이고 안정적으로 학습시키는 고도화된 분산 트레이닝 기법을 전수합니다.
전통적인 HPC(고성능 컴퓨팅) 스케줄러인 Slurm은 자원이 100% 확보되어야 작업을 시작하는 '갱 스케줄링(Gang Scheduling)'에 최적화되어 있어, 대규모 GPU 자원을 관리할 때 쿠버네티스의 유연함과 Slurm의 안정성을 결합하는 것이 매우 중요합니다. 이때 Slinky(슬린키)는 쿠버네티스의 작업 요청을 Slurm에 전달하고, 할당된 자원을 포드(Pod)에 정밀하게 매핑하는 가교 역할을 수행합니다.
특히 이번 워크숍에서는 Slinky의 여러 모델 중 '쿠버네티스 포드 형태 그대로 Slurm의 GPU 자원을 투명하게 사용하는 방식'에 집중합니다. 이를 통해 참가자들은 별도의 복잡한 설정 없이도 익숙한 쿠버네티스 환경 내에서 Slurm의 강력한 스케줄링 성능을 마스터하고, 최적의 인프라 효율을 구현하는 실전 노하우를 습득할 수 있습니다.
